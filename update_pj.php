...
<head>
    ...
    <title>ลบโครงงาน | เว็บจับเก็บฐานข้อมูล</title>
...
background-color: #FFFFAA
        ...
        <div class="container"> <a class="navbar-brand" href="home.php"><img src="img/assets/home.png" width="30" height="30" alt="" class="d-inline-block align-top">&nbspเว็บไซต์จัดเก็บโครงงาน</a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item active"> <a class="nav-link" href="update_pj.php">ลบโครงงาน</a></li>
                </ul>
    ...
    <div class="col-md-8 col-sm-6 mx-auto mt-5">
        <form action="" method="post" enctype="multipart/form-data">
            <div class="form-inline">
                <label for="search_namepj" class="mr-2 col-form-label"><h4>ชื่อโครงงาน</h4></label>
                <style>
                    input[type=text],
                    select,
                    textarea {
                ...
                <input type="text" class="col-6 col-lg-9" name="search_namepj">
                ...
                <table class="table">
                    <thead class="bg-success">
                        <tr>
                            ...
                            <th scope="col" class="text-center">ชื่อโครงงาน</t>
                    ...
                    <tbody>
                        ...
                        if (!isset($_REQUEST['search_namepj'])) { // ถ้ายังไม่ได้ request จากค้นหา
                            $sql = "SELECT `id`,`name_pj`,`makedate`,`link_pj` FROM `tbl_member`";
                        ...
                        ?>
                                <tr class="table-secondary">
                                    ...
                                    <td class="d-flex justify-content-center"><a onclick="cf('<?php echo $data['id'] ?>', '<?php echo $data['link_pj']?>')"><input type="button" value="ลบ" class="button1"></a></td>
                                </tr>
                            <?php
                            }
                        } else {
                            $search = $_REQUEST['search_namepj'];
                            $sql = "SELECT `id`,`name_pj`,`makedate`,`link_pj` FROM `tbl_member` WHERE `name_pj` LIKE '%$search%'";
                            $query = mysqli_query($conn, $sql);
                            $i = 0;
                            while ($data = mysqli_fetch_array($query)) {
                                $i++;
    ...
    <script>
        function cf() {
            var q = confirm("คุณต้องการลบไฟล์โครงงานหรือไม่");
            if (q)
                location.href = "delete.php?p=" + arguments[0] + "&link=" + arguments[1];
            else
                return false
        }
    </script>
